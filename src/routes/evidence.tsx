  ...
  head: () => ({
    meta: [
      { title: "Evidence — CustomerFirst OS" },
      {
        name: "description",
        content:
          "Every evidence record with an explicit strength, so weak evidence never looks like strong evidence.",
      },
      { property: "og:title", content: "Evidence — CustomerFirst OS" },
      {
        property: "og:description",
        content: "Evidence records, strengths and what they support or challenge.",
      },
    ],
  }),
  component: Evidence,
  ...
  return (
    <Shell>
      <PageHeader
        ...
        title="Evidence"
        lede="Evidence may support or challenge a hypothesis, a decision, an outcome or a pattern. Strength is always explicit."
      />
      <div className="mx-auto max-w-7xl px-6 py-10">
        <p className="text-sm text-muted-foreground">
          {e.length} records · {count("Strong")} strong · {count("Moderate")} moderate ·{" "}
          {count("Weak")} weak
        </p>
          ...
          {e.map((x) => {
            ...
            return (
              <li key={x.id} className="p-5">
                ...
                <p className="text-sm text-muted-foreground">
                  Supports: {x.supports.join(", ") || "—"} · source: {x.source}
                </p>
              </li>
            );
          })}
