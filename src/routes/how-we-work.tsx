...
export const Route = createFileRoute("/how-we-work")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Principles, operating model, delivery lifecycle, practices, governance and templates — all canonical Markdown held in GitHub." },
      { property: "og:title", content: "How we work — CustomerFirst OS" },
      { property: "og:description", content: "The CustomerFirst operating manual, written in Markdown and reviewed through Git." },
    ],
  }),
  component: HowWeWork,
});
            ...
            <ul className="mt-6 grid gap-px overflow-hidden rounded-xl border bg-border md:grid-cols-2 lg:grid-cols-3">
              {group.docs.map((doc) => (
                <li key={doc.id} className="bg-card">
                  <Link to="/how-we-work/$" params={{ _splat: doc.slug }} className="block h-full p-6 hover:bg-surface">
