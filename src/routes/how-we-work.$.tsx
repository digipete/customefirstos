...
export const Route = createFileRoute("/how-we-work/$")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData)
      return {
        meta: [
          { title: "Guidance unavailable — CustomerFirst OS" },
  ...
  errorComponent: () => (
    <Shell>
      <div className="mx-auto max-w-3xl px-6 py-24">
        <h1 className="text-2xl font-semibold">This guidance didn't load</h1>
      </div>
    </Shell>
  ),
  notFoundComponent: () => (
    <Shell>
      <div className="mx-auto max-w-3xl px-6 py-24">
        ...
        <Link to="/how-we-work" className="mt-4 inline-block underline">
          Back to How we work
        </Link>
      </div>
    </Shell>
  ...
  return (
    <Shell>
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-12 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <article>
          <Link to="/how-we-work" className="text-sm text-muted-foreground underline">
            ← How we work
          </Link>
          ...
          <div>
            ...
            <dl className="mt-3 space-y-1 text-muted-foreground">
              <div>
                ...
                <dd className="inline font-mono">{doc.id}</dd>
              </div>
              <div>
                <dt className="inline font-medium text-foreground">Owner: </dt>
                <dd className="inline">{doc.owner?.profession}</dd>
              </div>
              <div>
                <dt className="inline font-medium text-foreground">Version: </dt>
                <dd className="inline">{String(doc.version)}</dd>
              </div>
              <div>
                <dt className="inline font-medium text-foreground">Last reviewed: </dt>
                <dd className="inline">{doc.reviewed}</dd>
              </div>
            </dl>
            <a
              href={doc.githubPath}
              className="mt-4 inline-block rounded-md bg-primary px-3 py-2 text-primary-foreground"
              rel="noreferrer"
            >
