---
id: cf-decision-framework
title: Decision Framework
description: How CustomerFirst makes, records and reviews significant decisions.
type: practice
section: decisions
status: live
maturity: established
owner:
  profession: Leadership
version: 0.4
created: 2026-02-10
reviewed: 2026-08-14
applies_to:
  - discovery
  - define
  - experiment
tags:
  - decisions
  - governance
related:
  - cf-evidence-standard
  - cf-minimum-viable-governance
  - cf-architecture-assurance
  - cf-principle-smallest-safe-decision
---

# Decision Framework

A significant decision is one that is expensive or slow to reverse.

```mermaid
flowchart LR
Q[Question] --> E[Evidence] --> O[Options] --> T[Trade-offs] --> D[Decision] --> R[Review]
R -.-> Q
```

## What this means for us

Every significant decision is recorded with the question, the evidence considered, the options, the trade-offs accepted, the owner and a review date. Decisions are searchable across the whole of CustomerFirst.

## Good looks like

- the question written before the answer
- at least one genuinely viable alternative option
- trade-offs stated as costs we accept, not risks we note
- a review date that someone actually honours

## Anti-patterns

- decisions recorded after the fact to justify a direction
- options lists containing one real option and two straw men
- decision latency measured in months because no owner was named

## Assurance prompt

What evidence supports increasing our exposure? Do not make a bigger decision than the evidence allows.
